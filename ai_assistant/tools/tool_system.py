...
class ToolSystem:
    ...
    def __init__(self, tool_registry_file: Optional[str]=None):
        ...
        # Explicit list of custom tool modules to discover
        self.custom_tool_modules_to_discover = [
            ('ai_assistant.custom_tools.agent_tools', 'agent_tools.py'),
            ('ai_assistant.custom_tools.architect_tools', 'architect_tools.py'),
            ...
            ('ai_assistant.custom_tools.file_editing_tools', 'file_editing_tools.py'),
            ('ai_assistant.custom_tools.file_system_tools', 'file_system_tools.py'),
            ('ai_assistant.custom_tools.git_tools', 'git_tools.py'),
            ('ai_assistant.custom_tools.introspection_tools', 'introspection_tools.py'),
            ('ai_assistant.custom_tools.knowledge_tools', 'knowledge_tools.py'),
            ('ai_assistant.custom_tools.memory_tools', 'memory_tools.py'),
        ...
        # The previous logic only ran it if registry was empty, which is bad for updates
        # We will make it smart: discover, and if we find new ones, save.
        any_new_tools_registered_overall = False

        if is_debug_mode():
            print('ToolSystem: Running full tool discovery scan...')
        ...
        for module_import_path, module_filename in self.custom_tool_modules_to_discover:
            try:
                ...
                if self._discover_and_register_custom_tools(module_to_inspect, module_import_path):
                    if is_debug_mode():
                        print(f'ToolSystem: New/Updated tools discovered from {module_filename}.')
                    any_new_tools_registered_overall = True
            except ImportError as e:
                if is_debug_mode():
                    print(f'ToolSystem: Could not import {module_filename} (path: {module_import_path}): {e}')
    ...
    def _discover_and_register_custom_tools(self, module_to_inspect, module_path_str: str) -> bool:
        ...
        for name, func_object in inspect.getmembers(module_to_inspect, inspect.isfunction):
            ...
            # We want to re-register if the description changed (handled by register_tool logic internally partially)
            # but for now, let's force re-registration attempt if schema is found to ensure metadata updates
            ...
            try:
                ...
                # Should not happen with current register_tool implementation which overwrites
                if is_debug_mode():
