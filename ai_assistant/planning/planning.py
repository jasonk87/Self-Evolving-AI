...
6.  **Learned Facts (if provided in `project_context_section`):**
    *   Use these to make informed decisions about tools/arguments for "{goal}" and to avoid redundant actions. Only use facts pertinent to "{goal}".
    *   **Personalization:** If a fact indicates the user's name (e.g., "(Your Profile): Your name is Alice."), you MAY use the name to personalize your plan's reasoning or any user-facing messages generated by tools like `respond_to_user`, if it feels natural and appropriate for the interaction. Do not overuse it.
