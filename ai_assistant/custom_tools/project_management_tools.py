...
from ai_assistant.custom_tools.file_system_tools import create_project_directory, write_text_to_file, sanitize_project_name, BASE_PROJECTS_DIR, read_text_from_file
import logging
logger = logging.getLogger(__name__)
...
async def initiate_ai_project(project_name: str, project_description: str=None, description: str=None) -> str:
    ...
    try:
        ...
        for i, file_dict in enumerate(raw_plan_list):
            ...
            if not (filename and isinstance(filename, str) and description and isinstance(description, str)):
            ...
            dev_task = DevelopmentTask(task_id=task_id, task_type='CREATE_FILE', description=f'Define structure and generate code for {filename}', details={'filename': filename, 'original_description': description, 'key_components': key_components, 'file_dependencies': dependencies}, status='planned')
    ...
    manifest_instance = ProjectManifest(project_name=project_name, sanitized_project_name=sanitized_name, project_directory=project_dir_path, project_description=project_description, creation_timestamp=current_timestamp_iso, last_modified_timestamp=current_timestamp_iso, manifest_version='1.1.0', version='0.1.0', project_type='python', development_tasks=development_tasks_list, entry_points={}, dependencies=[], project_goals=[], build_config=BuildConfig(), test_config=TestConfig())
    ...
    return f"Success: Project '{project_name}' initialized in '{project_dir_path}' with src, tests dirs and README.md. Plan includes {plan_summary_str}. You can now ask to generate code for these tasks."
...
async def generate_code_for_project_file(project_name: str, filename: str=None, file_path: str=None, **kwargs) -> str:
    ...
    if file_task_entry:
        ...
        if isinstance(key_components_str, list):
            key_components_str = '\n'.join([f'- {s}' for s in key_components_str])
        if isinstance(dependencies_str, list):
            dependencies_str = ', '.join([str(s) for s in dependencies_str])
    prompt = CODE_GENERATION_PROMPT_TEMPLATE.format(project_description=overall_project_desc, filename=filename, file_description=file_plan_description, key_components_str=key_components_str, dependencies_str=dependencies_str)
...
async def add_dependency_to_project(project_name: str, dependency_name: str, dependency_version: Optional[str]=None, dependency_type: Optional[str]=None) -> str:
    if not project_name or not isinstance(project_name, str) or (not project_name.strip()):
        return 'Error: Project name must be a non-empty string.'
    ...
    new_dependency = Dependency(name=dependency_name, version=dependency_version, type=dependency_type)
    ...
    if existing_dep_index != -1:
        ...
        if new_dependency.version is not None and current_dep.version != new_dependency.version or (new_dependency.type is not None and current_dep.type != new_dependency.type) or (new_dependency.version is None and current_dep.version is not None) or (new_dependency.type is None and current_dep.type is not None):
...
async def run_project_tests(project_name: str) -> str:
    ...
    try:
        ...
        process_result = subprocess.run(test_command_parts, cwd=project_dir, capture_output=True, text=True, timeout=300, check=False)
        output_summary = [f'Test Results for Project: {project_name}', f'Command: {test_command_str}', f'Return Code: {process_result.returncode}', '--- STDOUT ---', process_result.stdout.strip() if process_result.stdout else '(No standard output)', '--- STDERR ---', process_result.stderr.strip() if process_result.stderr else '(No standard error)']
        return '\n'.join(output_summary)
...
async def build_project(project_name: str) -> str:
    ...
    try:
        ...
        process_result = subprocess.run(build_command_parts, cwd=project_dir, capture_output=True, text=True, timeout=600, check=False)
        output_summary = [f'Build Results for Project: {project_name}', f'Command: {build_command_str}', f'Return Code: {process_result.returncode}', '--- STDOUT ---', process_result.stdout.strip() if process_result.stdout else '(No standard output)', '--- STDERR ---', process_result.stderr.strip() if process_result.stderr else '(No standard error)']
        return '\n'.join(output_summary)
    except FileNotFoundError:
...
from ai_assistant.core.self_modification import edit_project_file
from ai_assistant.core.task_manager import TaskManager

async def propose_project_file_update(absolute_target_filepath: str, new_file_content: str, change_description: str, task_manager: Optional[TaskManager]=None, parent_task_id: Optional[str]=None) -> Dict[str, Any]:
    ...
    try:
        result_message = await edit_project_file(absolute_file_path=absolute_target_filepath, new_content=new_file_content, change_description=change_description, task_manager=task_manager, parent_task_id=parent_task_id)
...
if __name__ == '__main__':
    ...
    async def _mock_invoke_ollama_planner(prompt: str, model_name: str, **kwargs):
        if 'Detailed Plan Test' in prompt:
            return json.dumps({'project_plan': [{'filename': 'app.py', 'description': 'Main app.', 'key_components': ['comp1'], 'dependencies': ['util.py']}, {'filename': 'util.py', 'description': 'Utilities.', 'key_components': ['helper'], 'dependencies': []}]})
    ...
    _original_builtin_print = __builtins__.print
    ...
    async def run_tests():
        ...
        __builtins__.print = _captured_print
        ...
        _original_builtin_print(f'Result 1: {result_1}')
        assert 'Success' in result_1
        assert '2 development task(s)' in result_1
        ...
        assert any(("key_components' for file 'core.py' is not a list" in warn for warn in _printed_warnings_for_test))
        assert any(("dependencies' for file 'core.py' is not a list" in warn for warn in _printed_warnings_for_test))
        ...
        assert 'empty plan' in result_4.lower()
        ...
        __builtins__.print = _original_builtin_print
    ...
    async def run_pm_main_tests():
        ...
        with tempfile.TemporaryDirectory() as temp_dir_for_propose_main:
            ...
            result_prop5_main = await propose_project_file_update(test_file_path_for_propose_main, 'some content', '')
...
async def request_code_review_tool(code_to_review: Optional[str]=None, original_requirements: Optional[str]=None, related_tests: Optional[str]=None) -> Dict[str, Any]:
    """
