...
async def search_duckduckgo(*args, **kwargs) -> str:
    ...
    if not results:
        ...
        try:
            # Await the wrapper (which awaits the primary tool)
            google_res_json = await search_google_custom_search(query, num_results=5)
...
async def search_google_custom_search(query: str, num_results: Union[int, str]=5) -> str:
    ...
    try:
        ...
        results_list = await primary_google_search(query, num_results=parsed_num)
        
        if results_list is None:
            return '[]'
            
        # Standardize keys if needed (primary tool returns 'link', 'snippet'; this one returned 'href', 'body'?)
        # Primary returns: title, link, snippet
        # This function historically returned: title, href, body
        # We must map them to avoid breaking callers.
        mapped_results = []
