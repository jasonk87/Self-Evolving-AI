    ...
    with tempfile.TemporaryDirectory() as temp_dir_path:
        ...
        try:
            ...
            if use_docker:
                ...
                docker_timeout = timeout_seconds + 5 # Give 5s buffer for docker startup
                
                # We use python:3.11-slim as a lightweight deterministic environment
                # Network disconnected for safety. Resource constraints can be added here.
                docker_cmd = [
                    "docker", "run", "--rm",
                    "--network", "none",      # Disconnect from internet
                    "--memory", "512m",       # Prevent OOM bombs
                    "--cpus", "1.0",          # Limit CPU
                    "-v", f"{os.path.abspath(temp_dir_path)}:/sandbox",
                    "-w", "/sandbox",
                    "python:3.11-slim",
...
def search_codebase(query: str, directory: str = ".", include_globs: Optional[List[str]] = None, case_sensitive: bool = False) -> Dict[str, Any]:
    ...
    Args:
        query: The regex string to search for.
