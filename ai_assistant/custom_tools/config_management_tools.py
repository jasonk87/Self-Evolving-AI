...
def manage_tool_confirmation_settings(action: str, tool_name: Optional[str]=None) -> str:
    ...
    try:
        ...
        non_system_internal_tool_names = [name for name, data in tool_system_instance._tool_registry.items() if data.get('type') != 'system_internal']
    except ImportError:
    ...
    if action == 'add_all':
        if non_system_internal_tool_names is not None:
            updated_list = sorted(list(set(non_system_internal_tool_names)))
    ...
    if action == 'remove_all':
        ...
        if _save_requires_confirmation_list([]):
...
if __name__ == '__main__':
    ...
    class MockToolSystemInstanceForConfigTest:
        ...
        def __init__(self):
            self._tool_registry = {'search_duckduckgo': {'description': 'Searches the web.', 'type': 'custom_discovered'}, 'get_self_awareness_info_and_converse': {'description': 'Provides info about the AI.', 'type': 'custom_discovered'}, 'another_example_tool': {'description': 'Does something else.', 'type': 'custom_discovered'}, 'project_tool_A': {'description': 'Manages project A.', 'type': 'custom_discovered'}, 'system_update_tool_metadata': {'description': 'Internal system tool.', 'type': 'system_internal'}}
