...
def _handle_delegation_commands(session_id: str, message: str, images=None):
    ...
    if stripped.startswith('/work-inbox'):
        ...
        if arg_lower == 'open' or arg_lower.startswith('open ') or arg_lower == 'retry' or arg_lower.startswith('retry '):
            ...
            all_notices = app_globals.chat_manager.list_user_notices(DEFAULT_NOTICE_SCOPE, include_read=True, limit=200)
            ...
            if action == 'retry':
                ...
                app_globals.chat_manager.mark_user_notices_read(DEFAULT_NOTICE_SCOPE, notice_ids=[notice_id])
...
def chat():
    ...
    if not session_id:
        if identity_key and hasattr(app_globals.chat_manager, 'get_or_create_session_for_identity'):
            session_id = app_globals.chat_manager.get_or_create_session_for_identity(identity_key)
        else:
