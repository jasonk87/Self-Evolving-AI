...
def test_emit_task_failure_alert_ignores_failed_interrupted_status(monkeypatch):
    task = _make_task(status=ActiveTaskStatus.FAILED_INTERRUPTED)
...
def test_emit_task_failure_alert_ignores_internal_tool_modification_precondition(monkeypatch):
    task = ActiveTask(
        description="Executing action: PROPOSE_TOOL_MODIFICATION",
        task_type=ActiveTaskType.AGENT_TOOL_MODIFICATION,
        status=ActiveTaskStatus.FAILED_PRE_REVIEW,
        status_reason="Missing module_path or function_name for tool 'Agent Spawner'. Cannot attempt modification.",
        current_step_description="Precondition check failed",
    )
...
def test_emit_startup_interrupted_tasks_digest_emits_socket_without_chat_message(monkeypatch):
    ...
    socketio = SimpleNamespace(emit=lambda event, payload: emitted.update({"event": event, "payload": payload}))

    monkeypatch.setattr(app_globals, "chat_manager", chat_mgr)
    monkeypatch.setattr(app_globals, "socketio", socketio)

    payload = conversational_alerts.emit_startup_interrupted_tasks_digest([task_a, task_b])
