...
def test_chat_delegate_code_command_creates_task(monkeypatch):
    ...
    def fake_launch(task_id, session_id, delegated_prompt, history_snapshot, user_scope=None):
        ...
        launched["user_scope"] = user_scope
...
def test_chat_work_inbox_persists_across_rotated_session_for_same_identity(monkeypatch):
    app = _build_test_app()
    ...
    # Create a completion notice tied to telegram identity scope.
    app_globals.chat_manager.add_user_notice(
        "telegram:u11:c11",
        "Delegated task cross-session completed.",
        source_session_id="s1",
        task_id="TASKX123",
        status="completed",
    )
    ...
    with app.test_client() as client:
        first = client.post('/chat', json={"message": "/work-inbox", "platform": "telegram", "user_id": "u11", "chat_id": "c11"})
        restarted = client.post('/chat', json={"message": "/start", "platform": "telegram", "user_id": "u11", "chat_id": "c11"})
        second = client.post('/chat', json={"message": "/work-inbox", "platform": "telegram", "user_id": "u11", "chat_id": "c11"})

    assert first.status_code == 200
    assert restarted.status_code == 200
    assert second.status_code == 200
    assert "TASKX123"[:8] in first.get_json()["response"]
    assert "TASKX123"[:8] in second.get_json()["response"]


def test_chat_work_inbox_isolated_between_identities(monkeypatch):
    app = _build_test_app()
    ...
    app_globals.chat_manager.add_user_notice(
        "telegram:user_a:chat_a",
        "Delegated task only for user A.",
        source_session_id="s1",
        task_id="A123",
        status="completed",
    )
    ...
    with app.test_client() as client:
        user_a = client.post('/chat', json={"message": "/work-inbox", "platform": "telegram", "user_id": "user_a", "chat_id": "chat_a"})
        user_b = client.post('/chat', json={"message": "/work-inbox", "platform": "telegram", "user_id": "user_b", "chat_id": "chat_b"})

    assert user_a.status_code == 200
    assert "A123" in user_a.get_json()["response"]
    assert user_b.status_code == 200
    assert "No work notices yet." in user_b.get_json()["response"]



def test_chat_identity_continuity_overrides_mismatched_session_id(monkeypatch):
    app = _build_test_app()
    ...
    with app.test_client() as client:
        first = client.post('/chat', json={"message": "/work-inbox", "platform": "telegram", "user_id": "u2", "chat_id": "c2"})
        second = client.post('/chat', json={"message": "/work-inbox", "platform": "telegram", "user_id": "u2", "chat_id": "c2", "session_id": "stale_session"})
...
def test_chat_identity_continuity_uses_rotated_session_even_if_old_session_id_provided(monkeypatch):
    app = _build_test_app()
    ...
    with app.test_client() as client:
        before = client.post('/chat', json={"message": "/work-inbox", "platform": "telegram", "user_id": "u3", "chat_id": "c3"})
        restarted = client.post('/chat', json={"message": "/start", "platform": "telegram", "user_id": "u3", "chat_id": "c3"})
        after = client.post('/chat', json={"message": "/work-inbox", "platform": "telegram", "user_id": "u3", "chat_id": "c3", "session_id": before.get_json()["session_id"]})

    assert before.status_code == 200
    assert restarted.status_code == 200
    assert after.status_code == 200
    assert restarted.get_json()["session_id"] != before.get_json()["session_id"]
    assert after.get_json()["session_id"] == restarted.get_json()["session_id"]
