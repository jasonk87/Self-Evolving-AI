...
from ai_assistant.custom_tools import calendar_tools
...
def test_check_calendar_validates_before_authentication(monkeypatch):
    monkeypatch.setattr(
        calendar_tools,
        "_get_manager",
        lambda: (_ for _ in ()).throw(AssertionError("calendar manager should not initialize")),
    )

    assert calendar_tools.check_calendar("").startswith("Error: Date must")
    assert calendar_tools.check_calendar(None).startswith("Error: Date must")


def test_check_calendar_normalizes_date(monkeypatch):
    class Manager:
        def get_day_agenda(self, date_str):
            return date_str

    monkeypatch.setattr(calendar_tools, "_get_manager", lambda: Manager())

    assert calendar_tools.check_calendar("  tomorrow  ") == "tomorrow"
